...
```bash
# a checkpoint from the other family
...
git, and about 12 GiB of disk — most of it for the ONNX export, which the s3
models skip because they are released as graphs.
...
- **Web console** for trying it: synthesis, live monitoring, and uploading your
  own voices where the model can learn one. Compiled into the binary.
- **NUMA placement**: one engine per node, weights replicated, threads pinned,
  memory bound locally.
- **Two backends** behind one interface, chosen by `architecture` in the bundle
  manifest. Everything above that line is shared.
- **Stress marks** through a RUAccent sidecar for Russian, used by both
  architectures — Pocket wants a combining acute, s3 wants `+` before the vowel,
  and the client writes whichever is convenient. Without stress the Russian
  Pocket checkpoint scores 17.7% WER against 3.2%.

What actually differs:

| | `pocket` | `s3` |
|---|---|---|
| shape | autoregressive over a 12.5 Hz codec | duration predictor + one flow-matching pass |
| TTFB | flat in text length | grows with it |
| voices | prefilled KV cache, clonable from a WAV | fixed style vectors, not clonable |
| graphs | five, exported from weights | four, shipped as ONNX |
| output | 24 kHz | 44.1 kHz |
...
## How the pocket export is put together

The s3 checkpoints ship their graphs, so this is about Pocket TTS only;
`tools/export/fetch_s3.py` downloads the others and writes their manifest.
...
## Cloning references (pocket)

Only the autoregressive family can learn a voice from audio; the s3 checkpoints
ship fixed style vectors and refuse the upload.
...
src/engine/    backend.hpp (the seam), engine.cpp (pocket), s3.cpp, factory
src/text/      stress, normalisation, token chunking, the s3 text path,
...
tools/export/  ONNX export and quantisation for pocket, fetch_s3.py for the
               checkpoints that ship graphs, Python reference runtime
